...
@Component({
    selector   : 'album-shortcuts',
    templateUrl: './shortcuts.component.html',
    styleUrls  : ['./shortcuts.component.scss']
})
export class AlbumShortcutsComponent implements OnInit, OnDestroy
{
    shortcutItems: any[];
    navigationItems: any[];
    ...
    ngOnInit(): void
    {
        ...
        this.filteredNavigationItems = this.navigationItems = this._albumNavigationService.getFlatNavigation(this.navigation);

        const cookieExists = this._cookieService.check('ALBUM2.shortcuts');

        if ( cookieExists )
        {
            this.shortcutItems = JSON.parse(this._cookieService.get('ALBUM2.shortcuts'));
        }
        else
        {
    ...
    toggleShortcut(event, itemToToggle): void
    {
        ...
        for ( let i = 0; i < this.shortcutItems.length; i++ )
        {
            if ( this.shortcutItems[i].url === itemToToggle.url )
            {
                ...
                // Save to the cookies
                this._cookieService.set('ALBUM2.shortcuts', JSON.stringify(this.shortcutItems));
